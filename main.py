...
def sparkline(data):
    ...
    span = mx - mn or 1
    return "".join(
        SPARK_BARS[int((v - mn) / span * (len(SPARK_BARS) - 1))]
        for v in data
    )
...
# ───────── Helpers for Meta Ads (corrected) ─────────
...
async def export_google_active_campaigns_csv(
    ...
    review    = (
        ...
        f"{total_imp} impressões, {total_clk} cliques | CTR {avg_ctr}%"
    )
...
async def export_meta_active_campaigns_csv(
    ...
    review    = (
        ...
        f"{total_imp} impressões, {total_clk} cliques, spend {total_spd} | "
    ...
    w.writerow(["Average CTR (%)",   avg_ctr])
    w.writerow(["Average CPC",       avg_cpc])
    w.writerow([])
...
async def export_combined_active_campaigns_csv(
    ...
    # trends combined (just Google trend for sparkline)
    dates, imps, clks = await google_ads_list_trends(google_refresh_token)
    ...
    w.writerow(["Total Impressions",  total_imp])
    w.writerow(["Total Clicks",       total_clk])
    w.writerow(["Overall CTR (%)",    avg_ctr])
    w.writerow([])
